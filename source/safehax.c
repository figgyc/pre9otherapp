...
#define DEBUG(x)   drawTitleScreen(x);
...
void safehax(){
	...
	payload_size = FileRead(payload_buf, "sdmc:/p9a9.bin", 0xFF000);
	PANIC(!payload_size, "FAILED TO READ THE PAYLOAD!");
	
	DEBUG("Reading ARM11 payload...");
	PANIC(!FileRead(payload_buf + 0xFF000, "sdmc:/p9a11.bin", 0xE00), "FAILED TO READ THE ARM11 PAYLOAD!");
